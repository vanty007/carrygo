			...
			<ul data-submenu-title="Account">
				...
				<?php
					if(user_login_status() == true){
						...
						//echo 'hjj'.ROLE_ID;
				?>
				<li class="active"><a style="cursor: pointer;" onclick="fowner()"><i class="im im-icon-Post-Sign"></i>Property Upload Request</a></li>
				<?php
						}
						}
				?>
            ...
            <ul>
                ...
                <li class="pending-booking" v-for="(data,index) in records">
                    <div class="list-box-listing bookings">
                        <div class="list-box-listing-img" v-if="data.document != null"><niceimg :path="data.document" width="100" height="100" ></niceimg> </div>
                        <div class="list-box-listing-img" v-if="data.document == null"><img src="libb/assets/images/avatar_user.png" alt=""></div>
                        <div class="list-box-listing-content">
                            <div class="inner">
                                <h3>{{data.title}} {{data.lastname}} {{data.firstname}}
                                <span class="booking-status pending" v-if="data.status == 0">Pending Approval</span>
                                    ...
                                    <span class="booking-status unpaid" style="background-color: red;" v-if="data.status == 2">Restricted Canceled</span>

                                <div class="inner-booking-list">
                                    <h5>Company Name:</h5>
                                    <ul class="booking-list">
                                        <li class="highlighted">{{data.name}}</li>
                                    </ul>
                                </div>
                                ...
                                <div class="inner-booking-list">
                                    <h5>Contact Info:</h5>
                                    <ul class="booking-list">
                                        <li class="highlighted">Contact Name: {{data.contactname}}</li>
                                    </ul>
                                    <ul class="booking-list">
                                        <li class="highlighted">Contact Phone: {{data.contactphone}}</li>
                                    </ul>
                                    <ul class="booking-list">
                                        <li class="highlighted">Contact Address: {{data.address}}</li>
                                    </ul>
                                </div>		
                                ...
                                <div class="inner-booking-list">
                                    <h5>Referal:</h5>
                                    <ul class="booking-list">
                                        <li>{{data.referalname}}</li>
                                        <li>{{data.referalphoneno}}</li>
                                    </ul>
                                </div>
                    ...
                    <div class="buttons-to-right">
                        <a style="cursor: pointer;" @click="requestHandleReject(data.id)" v-if="data.status == 0" class="button gray reject"><i class="sl sl-icon-close" ></i> Reject</a>
                        ...
                        <a style="cursor: pointer;" @click="requestHandleApprove(data.id)" class="button gray approve" v-if="data.status == 0"><i class="sl sl-icon-check"></i> Approve</a>
                        <a style="cursor: pointer;" @click="requestHandleRestrict(data.id)" class="button gray approve" v-if="data.status == 1"><i class="sl sl-icon-check"></i> Restrict</a>
                        <a style="cursor: pointer;" @click="requestHandleReactivate(data.id)" class="button gray approve" v-if="data.status == 2"><i class="sl sl-icon-check"></i> Reactivate</a>
                    </div>
                </li>
...
<!-- Wrapper / End -->
    ...
    <script>
	var OwnerListComponent = Vue.component('ownerList', {
		template: '#ownerList',
		...
		methods:{
            ...
            requestHandleReject : function(id){
						var apiurl = setApiUrl('components/requestHandleProperty/'+id);
						this.$http.post( apiurl , JSON.parse('{"status":0}') , {emulateJSON:true} ).then(function (response) {
                    ...
                    requestHandleApprove : function(id){
						var apiurl = setApiUrl('components/requestHandleProperty/'+id);
						this.$http.post( apiurl , JSON.parse('{"status":1}') , {emulateJSON:true} ).then(function (response) {
							console.log(response)
							window.location.href = response.body;
							location.reload();
						},
                    ...
                    requestHandleRestrict : function(id){
						var apiurl = setApiUrl('components/requestHandleProperty/'+id);
						this.$http.post( apiurl , JSON.parse('{"status":2}') , {emulateJSON:true} ).then(function (response) {
							console.log(response)
							window.location.href = response.body;
							location.reload();
						},
                    ...
                    requestHandleReactivate : function(id){
						var apiurl = setApiUrl('components/requestHandleProperty/'+id);
