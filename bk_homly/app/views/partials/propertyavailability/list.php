    <template id="propertyavailabilityList">
    <div>
					...
					<ul>
						...
						<li v-for="(data,index) in records">
							<div class="list-box-listing">
								<div class="list-box-listing-img"><a style="cursor: pointer;"  @click="loadHref(data.property_id)"><img :src="data.thumbnail" alt=""></a></div>
								<div class="list-box-listing-content">
									<div class="inner">
										<h3><a style="cursor: pointer;" @click="loadHref(data.property_id)">{{data.type}}</a></h3>
										<span>&#8358; {{data.price}}</span> <span> / {{data.rooms}}</span>
										                                        
									</div>
								</div>
							</div>
							<div class="buttons-to-right">
								<a style="cursor: pointer;" @click="loadHrefmaintenance(data.property_id)" class="button gray"><i class="sl sl-icon-note"></i> Maintenance</a>
								<a style="cursor: pointer;" @click="loadHrefedit(data.property_id)" class="button gray"><i class="sl sl-icon-note"></i> Edit</a>
								<!--<a style="cursor: pointer;" class="button gray" @click="deleteRecord(data.property_id,index)" title="Delete This Record">
                                <span v-show="deleting != data.property_id"><i class="fa fa-times"></i></span>
                                <clip-loader :loading="deleting == data.property_id" color="#fff" size="14px"></clip-loader> Delete</a>-->
							</div>
						</li>
...
<!-- Wrapper / End -->
    ...
    <script>
	var PropertyavailabilityListComponent = Vue.component('propertyavailabilityList', {
		template: '#propertyavailabilityList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'propertyavailabilitylist',
			},
			apipath : {
				type : String,
				default : 'propertyavailability/list',
			},
			tablestyle: {
				type: String,
		...
		methods:{
			...
			loadHrefmaintenance : function(id){
			window.location.href = "#/propertymaintenance/edit/"+id;
			location.reload();
					}, 
