<template id="propertyreportList">
	<div>
		<div id="wrapper">
							...
							<ul id="responsive" >
								<li class="active"><a>Reports</a>
									<ul class="sub-menu">
										...
										<li onclick="fpropertysoa()"><a style="cursor: pointer;">Statement of Account</a></li>
									</ul>
								</li>
...
<!-- Header Container / End -->
							...
							<div class="col-sm-3 comp-grid" :class="setGridSize">&nbsp;
								<input @keyup.enter="dosearchReportDateRange()" v-model="price_slider_max" class="form-control" type="text" name="search" placeholder="Date To" id="price_slider_max"/>
									...
									<div class="col-sm-3 comp-grid" :class="setGridSize">Filter Based on Properties
										...
										<!--<div style="display:none;" id="tags_location">{{v_propertylistRecords}}</div>-->
										<!--<button v-if="showCheckProperties" @click="dosearchReportProperties()" class="button border fw margin-top-10" type="button"></button>-->
					...
					<div class="col-12">
						<div class="card my-4">
							...
							<div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
								<h6 class="text-white text-capitalize ps-3" style="font-size:13px;">Property Report</h6>
							</div>
							</div>
								...
								<div class="table-responsive p-0" v-if="records.length" ref="datatable">
									...
									<thead>
										<tr>
											...
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Property Lease Price</th>
									...
									<tbody>
										<tr v-for="(data,index) in records">
                                            ...
                                            <td v-if="data.status == '0' " >Available</td>
											<td v-if="data.status == '1' " >Not Available</td>
    ...
    <script>
	var PropertyreportListComponent = Vue.component('propertyreportList', {
		template: '#propertyreportList',
					...
					this.$http.get(url).then(function (response) {
						...
						if(data && data.records){
							...
							for(var i=0;i<data.records.length;i++){
								...
								arr_labels.push(data.records[i]['updated_at']);
								arr_value.push(data.records[i]['service_charge']);
							}
