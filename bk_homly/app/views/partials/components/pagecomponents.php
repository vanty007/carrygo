...
<template id="DataSelect">
	<div class="data-select">
		<select style="font-size:13px;" v-model="selected" :multiple="multiple" class="custom-select form-control">
			<option v-if="placeholder" style="font-size:13px;" value="">{{ placeholder }}</option>
			<option v-for="opt in options" style="font-size:13px;" v-if="opt.value != undefined" :value="opt.value">{{ opt.label }}</option>
			<option v-for="opt in options" style="font-size:13px;" v-if="opt.value == undefined">{{ opt }}</option>
		</select>
...
<template id="DataCheck">
	...
	<div class="panel-dropdown-content checkboxes categories panel-dropdown">
		<span class="load-indicator" v-if="loading"><clip-loader :loading="loading" size="20px"></clip-loader> </span>
		<template v-if="isbutton" >
		  <b-form-checkbox-group class="checkboxes" text-field="label" style="width: 100%" v-model="selected"
								 buttons
								 :stacked="btnvertical"
		...
		<template v-else v-for="opt in options">
			<label  v-if="opt.value || opt.label">
