<template id="propertyoccupancyList">
	<div>
		<div id="wrapper">
...
<!-- Header Container / End -->
					...
					<div class="col-12">
						<div class="card my-4">
							...
							<div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
								<h6 class="text-white text-capitalize ps-3" style="font-size:13px;">Property Occupancy</h6>
							</div>
							</div>
							<div class="card-body px-0 pb-2">
								<div class="table-responsive p-0" v-if="records.length" ref="datatable">
									...
									<thead>
										<tr>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Property Name</Data></th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Property Type</Data></th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Property Location</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Lease Price</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Service Charge</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Lease From</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Lease To</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Lease Type</th>
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Booking Status</th>
									...
									<tbody>
										<tr v-for="(data,index) in records">
											...
											<td>{{ data.validdatestart }}</td>
											<td>{{ data.validdateend }}</td>
                                            <td>{{data.frequency}} {{ data.rooms }}</td>
                                            <td v-if="data.status == '0' " >Booking Pending</td>
											<td v-if="data.status == '1' " >Booking Approved</td>
											<td v-if="data.status == '2' " >Booking Cancelled</td>
											<td v-if="data.chargestatus == '0' " >Payment Pending</td>
											<td v-if="data.chargestatus == '1' " >Payment Approved</td>
                                    	...
                                    	<tr>
                                          <td colspan="3" class="grand-total-label" style="font-weight: bold;">Total:</td>
                                          <td class="grand-total" style="font-weight: bold;">N{{ grandTotal_Amount }}</td>
										  <td class="grand-total" style="font-weight: bold;">N{{ grandTotal_ServiceCharge }}</td>
                                        </tr>
									</tbody>
									</table>
								</div>
    ...
    <script>
	var PropertyoccupancyListComponent = Vue.component('propertyoccupancyList', {
		template: '#propertyoccupancyList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'propertyoccupancy',
			},
			apipath : {
				type : String,
				default : 'propertyoccupancy/list',
			},
			exportbutton: {
				type: Boolean,
				default: false,
		...
		computed : {
         grandTotal_ServiceCharge() {
            let sum = 0;
            this.records.forEach(item => sum += parseInt(item.service_charge));
        return sum.toLocaleString();
        },
         grandTotal_Amount() {
            let sum = 0;
            this.records.forEach(item => sum += parseInt(item.price));
        return sum.toLocaleString();
        },
			pageTitle: function(){
					...
					this.$http.get(url).then(function (response) {
						...
						if(data && data.records){
							...
							for(var i=0;i<data.records.length;i++){
								...
								arr_labels.push(data.records[i]['validdatestart']);
								arr_value.push(data.records[i]['price']);
							}
