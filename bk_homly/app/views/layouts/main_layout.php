...
<link href="lib/assets/demo/demo.css" rel="stylesheet" />
	<style>
...
.myButton:active {
	...
	<body id="<?php echo $page_id ?>" class="transparent-header">
		...
		<script src="lib/assets/demo/demo.js"></script>
	  ...
	  function loaddashboard(arr_labels, arr_value)
	{
		//console.log(arr_value)
		const response = {chart_product_trivia_revenue: {labels: arr_labels,datasets: [arr_value]}};

		chart_labels1 = response['chart_product_trivia_revenue']['labels'];
		chart_data1 = response['chart_product_trivia_revenue']['datasets'][0].map(function(str) {return parseInt(str); });
...
$(function() {
	...
	//function datePicker() {
		...
		if(currentUrl.includes('propertylist/view'))
	  {
		setTimeout(function() {
			...
			console.log("invalidto: "+$('#invalidto').val()+" invalidfrom: "+$('#invalidfrom').val())


		var splitDates_invalidto = $('#invalidto').val().split(",");
		var splitDates_invalidfrom = $('#invalidfrom').val().split(",");
		let allowedRanges = [];
		let excludedDates = [];

		if(splitDates_invalidto=="0"){
		console.log("no invalidto")
		}
		else{
		for(var i=0; i<splitDates_invalidto.length; i++){
		var val_splitDates_invalidto = splitDates_invalidto[i].replace(/\s+/g, "").split("-");
		var start = val_splitDates_invalidto[0].split("/")[2]+"-"+val_splitDates_invalidto[0].split("/")[0]+"-"+val_splitDates_invalidto[0].split("/")[1];
		var end = val_splitDates_invalidto[1].split("/")[2]+"-"+val_splitDates_invalidto[0].split("/")[0]+"-"+val_splitDates_invalidto[0].split("/")[1];
		allowedRanges.push({start, end});
		}
		}
		if(splitDates_invalidfrom=="0"){
		console.log("no invalidfrom")
		}
		else{
		for(var i=0; i<splitDates_invalidfrom.length; i++){
		var val_splitDates_invalidfrom = splitDates_invalidfrom[i].replace(/\s+/g, "").split("-");
		var start = val_splitDates_invalidfrom[0].split("/")[2]+"-"+val_splitDates_invalidfrom[0].split("/")[1]+"-"+val_splitDates_invalidfrom[0].split("/")[0];
		var end = val_splitDates_invalidfrom[1].split("/")[2]+"-"+val_splitDates_invalidfrom[1].split("/")[1]+"-"+val_splitDates_invalidfrom[1].split("/")[0];

		excludedDates.push({start, end});
		}
	}

		allowedRanges.map(r => ({start: moment(r[0], "YYYY-MM-DD"),end: moment(r[1], "YYYY-MM-DD")}));
		excludedDates.map(d => ({start: moment(d[0], "YYYY-MM-DD"),end: moment(d[1], "YYYY-MM-DD")}));
    ...
    $('.date-picker').daterangepicker({
      ...
      isInvalidDate: function(date) {
        // First check if date is inside allowed ranges
        const inAllowedRange = allowedRanges.some(r =>
          date.isBetween(r.start, r.end, "day", "[]")
        );

        // Then check if it's in excluded dates
        const isExcluded = excludedDates.some(d =>
          date.isBetween(d.start, d.end, "day", "[]"));

        // Disable if not in allowed OR explicitly excluded
        return !inAllowedRange || isExcluded;
      }
    });

    // Show picked range only if valid
    $('.date-picker').on('apply.daterangepicker', function(ev, picker) {
      $(this).val(
		picker.startDate.format("DD/MM/YYYY") + "," + picker.endDate.format("DD/MM/YYYY")
        //picker.startDate.format("YYYY-MM-DD") + " to " + picker.endDate.format("YYYY-MM-DD")
      );
		var picker_set = $(ev.target).data('daterangepicker');
		if (picker_set === undefined) {
		
		}
		else{
		// contains the selected end date 
		var Difference_In_Time = new Date(picker_set.endDate._d).getTime() - new Date(picker_set.startDate._d).getTime();
