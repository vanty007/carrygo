...
class PropertylistController extends SecureController{
	...
	function view( $rec_id = null , $value = null){
		...
		$data->invalidto = $record[0]["invalidto"];

		if(count($propertyreservation)==1){
		$data->propertyreservation = $propertyreservation[0]['validdatestart'].'-'.$propertyreservation[0]['validdateend'];
		}
		else if(count($propertyreservation)>1){
		$str_join = $propertyreservation[0]['validdatestart'].'-'.$propertyreservation[0]['validdateend'];
		for($i=1;$i<count($propertyreservation);$i++){
