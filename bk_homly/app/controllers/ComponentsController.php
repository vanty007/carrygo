...
class ComponentsController extends BaseController{
    ...
    function get_owner_info(){
		$db = $this->GetModel();
		$user_id = USER_ID;
		$sqltext="SELECT status from owner where user_id = $user_id";
		$arr=$db->rawQueryOne($sqltext);
		
		render_json($arr) ;
	}

		function propertyfacility_facilityname_option_list($id){
		$db = $this->GetModel();
		$sqltext="SELECT  DISTINCT property_id AS value,facilities AS label FROM propertyavailability where property_id=$id";
		$arr=$db->rawQueryOne($sqltext);
		$arr_label = explode(',', $arr['label']);
		$arr_facilities = array();
		for($i=0; $i<count($arr_label);$i++){
			//"value" => $i;
    		//"label" => $arr_label[$i];
    		array_push($arr_facilities, array("value" => $arr_label[$i], "label" => $arr_label[$i]));
		}
		
		render_json($arr_facilities);
	}
	...
	function makeBooking(){
		...
		if(USER_ID==null){
			...
			if(count($get_validdate)==0){
		$db->insert('propertyreservation',array('validdateend'=>$validdateend,'validdatestart'=>$validdatestart,
