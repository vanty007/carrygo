 ...
 * @category  Controller
 */
class PropertyreportController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$value2="";
		$array_property_filter="";
		$fields = array('propertylist.id','propertylist.name','propertyavailability.type','propertylist.landmark','propertyavailability.price','propertyavailability.service_charge','propertyavailability.rooms','propertyavailability.facilities','propertyavailability.updated_at','propertyavailability.status');
		...
		if(isset($this->facilities_property) && strlen($this->facilities_property)>1){
			...
			//$array_property_filter = explode(",", 'ABA Houses, Oak Homes, Obasa');

			//if(count($array_property_filter)>1){
				
			for($i=0;$i<count($array_property_filter);$i++){
				$db->orWhere('propertylist.name', trim($array_property_filter[$i]));
			}	
			//$array_property_filter=$array_property_filter;
			/*}
			else{//('ABA Houses', 'Oak Homes', 'Obasa')
				//$db->orWhere('propertylist.name', $array_property_filter[0]);
				$db->orWhere('propertylist.name', 'ABA Houses');
				$db->orWhere('propertylist.name', 'Oak Homes');
				$db->orWhere('propertylist.name', 'Obasa');
			}*/
		}
		...
		if(!empty($this->orderby)){
			...
			$db->orderBy('propertylist.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$data->test = $array_property_filter;
