...
class PropertymaintenanceController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$user_id = USER_ID;
		$sql = "SELECT c1.property_id,c1.name,c1.facilityname,c1.frequency,c1.cost,c1.created_at
		 FROM propertyfacility as c1 
		INNER JOIN propertylist AS c2 ON c2.id = c1.property_id
		where c2.user_id = $user_id ORDER BY c1.id DESC";

		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(isset($this->facilities_property) && strlen($this->facilities_property)>1){
			$text=$this->facilities_property;
			$strQuery="(c1.name LIKE '%$text%' or c1.facilityname LIKE '%$text%' or c1.cost LIKE '%$text%' or c1.created_at LIKE '%$text%' )";

		$sql = "SELECT c1.property_id,c1.name,c1.facilityname,c1.frequency,c1.cost,c1.created_at
		 FROM propertyfacility as c1 
		INNER JOIN propertylist AS c2 ON c2.id = c1.property_id
		where c2.user_id = $user_id and $strQuery ORDER BY c1.id DESC";
		}
		...
		$strQuery="(c1.created_at BETWEEN '$this->price_slider_min' AND '$this->price_slider_max' )";
		...
		where c2.user_id = $user_id and $strQuery ORDER BY c1.id DESC";
			...
			if($this->what_property == "cost"){
		$sql = "SELECT c1.property_id,c1.name,c1.frequency,SUM(c1.cost)AS cost,c1.created_at
		 FROM propertyfacility as c1 
		INNER JOIN propertylist AS c2 ON c2.id = c1.property_id
		where c2.user_id = $user_id GROUP BY c1.property_id ORDER BY cost DESC";
			}
			else if($this->what_property == "costdate" && (isset($this->price_slider_min) && strlen($this->price_slider_min)>1) && (isset($this->price_slider_max) && strlen($this->price_slider_max)>1)){
				$strQuery="(c1.created_at BETWEEN '$this->price_slider_min' AND '$this->price_slider_max' )";
				$sql = "SELECT c1.property_id,c1.name,c1.facilityname,c1.frequency,SUM(c1.cost)AS cost,c1.created_at
				FROM propertyfacility as c1 
				INNER JOIN propertylist AS c2 ON c2.id = c1.property_id
				where c2.user_id = $user_id and $strQuery GROUP BY MONTH(c1.created_at) ORDER BY cost DESC";
			}
		}
			...
			for ($i = 0; $i < count($array_property_filter); $i++) {
				...
				$strQuery .= "C2.name = '$val' OR ";
			}
			
			$strQuery = preg_replace('/\s+OR\s*$/', '', $strQuery);
			
			$sql = "SELECT c1.property_id, c1.name, c1.facilityname, c1.frequency, c1.cost, c1.created_at
					FROM propertyfacility AS c1 
					INNER JOIN propertylist AS c2 ON c2.id = c1.property_id
					WHERE c2.user_id = $user_id AND ($strQuery)
					ORDER BY c1.id DESC";
		}
		if( !empty($fieldname) ){
