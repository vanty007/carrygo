...
class PaymentsController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$fields = array('payments.id','user.firstname','user.lastname', 'propertyreservation.status as b_status',	'propertyavailability.type', 'propertyavailability.price',	'propertyavailability.rooms', 	'payments.status', 	'payments.upload', 	'payments.amount', 	'payments.created_at');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text=$this->search;
			$db->orWhere('payments.id',"%$text%",'LIKE');
			$db->orWhere('payments.propertyavai_id',"%$text%",'LIKE');
			$db->orWhere('payments.user_id',"%$text%",'LIKE');
			$db->orWhere('payments.type',"%$text%",'LIKE');
			$db->orWhere('payments.status',"%$text%",'LIKE');
			$db->orWhere('payments.upload',"%$text%",'LIKE');
			$db->orWhere('payments.amount',"%$text%",'LIKE');
			$db->orWhere('payments.created_at',"%$text%",'LIKE');
		}
		$db->join("user","payments.user_id = user.auth_id","INNER");
		$db->join("propertyreservation","payments.reserve_id = propertyreservation.id","INNER");
		$db->join("propertyavailability","propertyreservation.propertyavailability_id = propertyavailability.id","INNER");
	...
	function view( $rec_id = null , $value = null){
		$db = $this->GetModel();
		$fields = array( 'id', 	'reserve_id', 	'user_id', 	'type', 	'status', 	'upload', 	'amount', 	'created_at' );
		if( !empty($value) ){
	...
	function add(){
		if(is_post_request()){
			...
			$reserve_id = $modeldata['id'];
			$property_id = $modeldata['property_id'];

			$sqltext = "SELECT a.id,a.email FROM auth a,propertylist p2 where a.id=p2.user_id and p2.id=$property_id order by p2.id desc limit 1";

			$get_property_user = $db->rawQueryOne($sqltext);

			$modeldata['user_id'] = USER_ID;
			$modeldata['reserve_id'] = $reserve_id;
			$modeldata['property_owner_id'] = $get_property_user['id'];
			...
			if(!empty($rec_id)){
				$db->where('id' , $modeldata['reserve_id']);
				$bool = $db->update('propertyreservation',array('chargestatus'=>1));

				$email = $get_property_user['email'];
				$verify_link=SITE_ADDR."#payments";
				$mailtitle="Payment Notification";
				$sitename=SITE_NAME;
				$bodymessage = "Please a user just made a payment for a booking on Homly.";
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			...
			$fields=array('id','reserve_id','user_id','type','status','upload','amount');
			$db->where('id' , $rec_id);
