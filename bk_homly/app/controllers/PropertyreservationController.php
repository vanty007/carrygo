<?php 
/**
 * Propertyreservation Page Controller
 * @category  Controller
 */
class PropertyreservationController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'property_id', 	'user_id', 	'rooms', 	'price', 	'propertyavailability_id', 	'created_at', 	'status');
		...
		if(!empty($this->search)){
			...
			$db->orWhere('propertyavailability_id',"%$text%",'LIKE');
		...
		$records = $db->get('propertyreservation', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'propertyreservation', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'propertyreservation' , false );
				}
				if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$rules_array = array(
				...
				'price' => 'required',
				'propertyavailability_id' => 'required|numeric',
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('id' , $rec_id);
			$bool = $db->update('propertyreservation',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
			...
			$fields=array('id','property_id','user_id','price','propertyavailability_id','status');
			$db->where('id' , $rec_id);
			$data = $db->getOne('propertyreservation',$fields);
			if(!empty($data)){
				render_json($data);
			}
			else{
	...
	function delete( $rec_ids = null ){
		...
		$bool = $db->delete( 'propertyreservation' );
		if($bool){
			render_json( $bool );
		}
		else{
