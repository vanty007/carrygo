...
class AuthController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'email', 	'login_system', 	'role_id', 	'profile_pics');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text=$this->search;
			$db->orWhere('id',"%$text%",'LIKE');
			$db->orWhere('email',"%$text%",'LIKE');
			$db->orWhere('login_system',"%$text%",'LIKE');
		...
		$records = $db->get('auth', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'auth', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'auth' , false );
				}
				if($db->getLastError()){
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			...
			$cpassword = $modeldata['confirm_password'];
			$new_password = $modeldata['new_password'];
			$password = $modeldata['password'];
			if($new_password == $password){
				render_error('Old password cannot be used');
			}
			...
			$db->where('id' , USER_ID);
			$data_password = $db->getOne('auth');

			if(!password_verify($password ,$data_password['password'])){
				render_error('Your current password is invalid');
			}
			
			$password_text = $modeldata['new_password'];
			$modeldata['password'] = password_hash($password_text , PASSWORD_DEFAULT);
			unset($modeldata['confirm_password']);
			unset($modeldata['new_password']);

			$db->where('id' , USER_ID);
			$bool = $db->update('auth', array('password'=>$modeldata['password']));
			if($bool){
				render_json('#/account/edit');
			}
			else{
				render_error($db->getLastError());
			}
			...
			$fields=array('id','login_system','role_id','profile_pics');
			$db->where('id' , USER_ID);
