 ...
 * @category  Controller
 */
class PropertyrevenueController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('payments.amount','payments.status', 'propertylist.id','propertylist.name','propertyavailability.type','propertylist.landmark',
		'propertyavailability.price','propertyavailability.service_charge','propertyavailability.rooms','propertyavailability.facilities','payments.created_at');
		...
		if(isset($this->facilities_property) && strlen($this->facilities_property)>1){
			...
			$db->orWhere('propertyavailability.type',"%$text%",'LIKE');
			...
			$db->Where('payments.created_at',[$this->price_slider_min,$this->price_slider_max],'between');//->Where('propertylist.created_at',"$this->price_slider_max",'<=');
		}
		...
		$db->join("payments","payments.reserve_id = propertyreservation.id","INNER");
		if(ROLE_ID == "super"){

		}
		else{
			$db->Where('payments.property_owner_id', USER_ID);
		}
