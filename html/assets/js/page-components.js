	...
	methods: {
		inputFile:function (newFile, oldFile) {
			if (newFile && oldFile && newFile.success && !oldFile.success) {
				...
				var msg = 'Total Rows Imported : <b>' + response.totalRows + '</b>';
				this.uploadMsg = msg;
				bus.$emit('refresh');
			}
...
var NiceFormWizard = Vue.component('NiceFormWizard',{
	...
	created: function(){
		...
		bus.$on('movewizard' , function(arr){
			var data = arr[0]; //submitted form data after validations
			var apipath = arr[1]; // submitted form url
			var action = arr[2]; // move wizard action (movenext | submitandmovenext)
			
			if(action == 'movenext'){
				// merge and keep form data and move to the next step
				vm.payload = extend( vm.payload, data ); 
				vm.$refs.wizard.nextTab();
			}
			else{
				// merge wizard steps form data and submit to the last form url
				var payload = extend( vm.payload, data ); //merge form data before submit
				vm.saving = true;
				vm.$http.post(apipath , payload, {emulateJSON:true}).then(function (response) {
					var result = response.body;
					vm.saving = false;
					vm.payload = {}; //reset wizard form data
					vm.$refs.wizard.nextTab();
				},
				function (response) {
					vm.saving = false;
...
var Pagination = Vue.component('Pagination', {
	...
	props: {
		...
		firstText : {
			type : String,
			default : '<i class="fa fa-angle-double-left"></i>',
		},
		nextText : {
			type : String,
			default : '<i class="fa fa-angle-right"></i>',
		...
		lastText : {
			type : String,
			default : '<i class="fa fa-angle-double-right"></i>',
		},
		prevText : {
			type : String,
