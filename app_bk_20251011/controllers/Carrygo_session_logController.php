<?php 
/**
 * Carrygo_Session_Log Page Controller
 * @category  Controller
 */
class Carrygo_Session_LogController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('tid', 	'session_id', 	'msisdn', 	'pcode', 	'session_date', 	'status', 	'session_duration_millis', 	'subid', 	'amount', 	'expirydate', 	'doiflag', 	'channel');
		...
		$records = $db->get('carrygo_session_log', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'carrygo_session_log', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'carrygo_session_log' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'carrygo_session_log', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('carrygo_session_log',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('tid' , $rec_id);
			$bool = $db->update('carrygo_session_log',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
			...
			$fields=array('tid','session_id','msisdn','pcode','session_date','status','session_duration_millis','subid','amount','expirydate','doiflag','channel');
			$db->where('tid' , $rec_id);
			$data = $db->getOne('carrygo_session_log',$fields);
			if(!empty($data)){
				render_json($data);
			}
			else{
	...
	function delete( $rec_ids = null ){
		...
		$bool = $db->delete( 'carrygo_session_log' );
		if($bool){
			render_json( $bool );
		}
		else{
