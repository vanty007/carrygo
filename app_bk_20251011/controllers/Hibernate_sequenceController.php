<?php 
/**
 * Hibernate_Sequence Page Controller
 * @category  Controller
 */
class Hibernate_SequenceController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$records = $db->get('hibernate_sequence', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'hibernate_sequence', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'hibernate_sequence' , false );
				}
				if($db->getLastError()){
...
// No View Function Generated Because No Field is Defined as the Primary Key on the Database Table
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('hibernate_sequence',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
