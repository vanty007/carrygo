<?php 
/**
 * Questions_Log_Seq Page Controller
 * @category  Controller
 */
class Questions_Log_SeqController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$records = $db->get('questions_log_seq', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'questions_log_seq', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'questions_log_seq' , false );
				}
				if($db->getLastError()){
...
// No View Function Generated Because No Field is Defined as the Primary Key on the Database Table
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('questions_log_seq',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
