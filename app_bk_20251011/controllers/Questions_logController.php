 ...
 * @category  Controller
 */
class Questions_LogController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($this->search)){
			...
			$db->orWhere('valid_option',"%$text%",'LIKE');
			$db->orWhere('quest_language',"%$text%",'LIKE');
			$db->orWhere('wildanswers',"%$text%",'LIKE');
		}
		...
		$records = $db->get('questions_log', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'questions_log', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
