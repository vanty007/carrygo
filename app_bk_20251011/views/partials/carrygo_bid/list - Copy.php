    <template id="carrygo_bidList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Carrygo Bid</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/carrygo_bid/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/carrygo_bid">Carrygo Bid</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/carrygo_bid/view/' +  data.id">{{data.id}}</router-link></td>
                                                    <td> {{ data.name }} </td>
                                                    <td><niceimg :path="data.image" width="50" height="50" ></niceimg> </td>
                                                    <td> {{ data.url }} </td>
                                                    <td> {{ data.price }} </td>
                                                    <td> {{ data.open_points }} </td>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="carrygo_bid/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var Carrygo_BidListComponent = Vue.component('carrygo_bidList', {
		template: '#carrygo_bidList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'carrygo_bidlist',
			},
			apipath : {
				type : String,
				default : 'carrygo_bid/list',
			},
			tablestyle: {
				type: String,
