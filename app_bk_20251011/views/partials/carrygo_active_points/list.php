    <template id="carrygo_active_pointsList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Carrygo Active Points</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/carrygo_active_points/add'">
                            <i class="fa fa-plus"></i>
                            Add New Carrygo Active Points
                            </router-link>
                        </div>
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/carrygo_active_points">Carrygo Active Points</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/carrygo_active_points/view/' +  data.id">{{data.id}}</router-link></td>
                                                    <td> {{ data.msisdn }} </td>
                                                    <td> {{ data.points }} </td>
                                                    <td> {{ data.updated_at }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/carrygo_active_points/view/' + data.id">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/carrygo_active_points/edit/' + data.id">
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="carrygo_active_points/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var Carrygo_Active_PointsListComponent = Vue.component('carrygo_active_pointsList', {
		template: '#carrygo_active_pointsList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'carrygo_active_pointslist',
			},
			apipath : {
				type : String,
				default : 'carrygo_active_points/list',
			},
			tablestyle: {
				type: String,
