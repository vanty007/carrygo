    <template id="session_logList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Session Log</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/session_log/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/session_log">Session Log</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/session_log/view/' +  data.tid">{{data.tid}}</router-link></td>
                                                    <td> {{ data.session_id }} </td>
                                                    <td> {{ data.msisdn }} </td>
                                                    <td> {{ data.last_command }} </td>
                                                    <td> {{ data.session_date }} </td>
                                                    <td> {{ data.status }} </td>
                                                    <td> {{ data.session_duration_millis }} </td>
                                                    <td> {{ data.message }} </td>
                                                    <td> {{ data.last_session_date }} </td>
                                                    <td> {{ data.network }} </td>
                                                    <td> {{ data.doiflag }} </td>
                                                    <td> {{ data.channel }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/session_log/view/' + data.tid">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/session_log/edit/' + data.tid">
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="session_log/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var Session_LogListComponent = Vue.component('session_logList', {
		template: '#session_logList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'session_loglist',
			},
			apipath : {
				type : String,
				default : 'session_log/list',
			},
			tablestyle: {
				type: String,
