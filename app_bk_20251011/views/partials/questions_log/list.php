<template id="questions_logList">
					...
					<div class="col-12">
						<div class="card my-4">
							...
							<div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
								<h6 class="text-white text-capitalize ps-3">Question and Answers</h6>
							</div>
							</div>
							<div class="card-body px-0 pb-2">
								<div class="table-responsive p-0" v-if="records.length" ref="datatable">
									...
									<thead>
										<tr>
										<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Content-Trivia No</Data></th>
										<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Category</Data></th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Question</Data></th>
									...
									<tbody>
										<tr v-for="(data,index) in records">
											<td><div style="font-size: 20px;text-wrap: balance;">{{ data.quest_language }}</div></td>
                                            <td>
												<span v-if="data.question_category  == '1'">Dogs</span>
												<span v-else-if="data.question_category  == '2'">Cats</span>
												<span v-else-if="data.question_category  == '3'">Fish</span>
												<span v-else-if="data.question_category  == '4'">Bird</span>
												<span v-else-if="data.question_category  == '5'">Cattle</span>
												<span v-else-if="data.question_category  == '6'">Domestic Animals</span>
											</td>
											<td><div style="font-size: 20px;text-wrap: balance;">{{ data.question }}</div></td>
                                            <td><div style="font-size: 20px;text-wrap: balance;">{{ data.wildanswers }}</div></td>
                                            <td>{{ data.valid_option }}</td>
                                                  <th class="td-btn">
                                                        <div >
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="contents/import_data" v-if="importbutton"></dataimport>
                                    </div>
    ...
    <script>
	var Questions_LogListComponent = Vue.component('questions_logList', {
		template: '#questions_logList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'questions_loglist',
			},
			apipath : {
				type : String,
				default : 'questions_log/list',
			},
			tablestyle: {
				type: String,
