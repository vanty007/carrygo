                ...
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Entries Log</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/entries_log/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/entries_log">Entries Log</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/entries_log/view/' +  data.tid">{{data.tid}}</router-link></td>
                                                    <td> {{ data.session_id }} </td>
                                                    <td> {{ data.msisdn }} </td>
                                                    <td> {{ data.first_question }} </td>
                                                    <td> {{ data.first_answer }} </td>
                                                    <td> {{ data.second_question }} </td>
                                                    <td> {{ data.second_answer }} </td>
                                                    <td> {{ data.third_question }} </td>
                                                    <td> {{ data.third_answer }} </td>
                                                    <td> {{ data.entrie_date }} </td>
                                                    <td> {{ data.last_answer_date }} </td>
                                                    <td> {{ data.question_category }} </td>
                                                    <td> {{ data.entry_info }} </td>
                                                    <td> {{ data.entry_qualification }} </td>
                                                    <td> {{ data.status }} </td>
                                                    <td> {{ data.location }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/entries_log/view/' + data.tid">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/entries_log/edit/' + data.tid">
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="entries_log/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
