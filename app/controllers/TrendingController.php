 ...
 * @category  Controller
 */
class TrendingController extends SecureController
{
	/**
	 * Index Action
	 * @return View
	...
	function index($fieldname = null, $fieldvalue = null)
	{
		...
		//$sql = "SELECT c1.id,c1.name,c1.image,c1.url,c1.price,c1.open_points,c1.open_date,c1.status,SUM(c2.points) bid_entry_points FROM carrygo_bid_entry c2, carrygo_bid c1 where c1.id=c2.bidid and c1.status = 0 GROUP BY c1.id ORDER BY bid_entry_points DESC";
		$sql = "SELECT c1.id,c1.name,c1.image,c1.url,c1.rating,c1.price,c1.open_points,c1.open_date,c1.status,SUM(c2.points) bid_entry_points,SUM(c3.points) bid_active_points
		...
		where c1.status = 0 or c1.status = 1 GROUP BY c1.id HAVING bid_entry_points>0 ORDER BY bid_entry_points DESC";
